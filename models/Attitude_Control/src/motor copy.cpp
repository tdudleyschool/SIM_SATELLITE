...
{
    ref_pos << r_pos[0], r_pos[1], r_pos[2];
    pos << 0, 0, 0;
    pos = pos + ref_pos;
    ref_pos_set = true;
}
...
{
    ref_ori << r_ori[0], r_ori[1], r_ori[2];
    ori << 0, 0, 0;
    ori = ori + ref_ori;
    ref_ori_set = true;
}
...
void motor::update_R_matrix(const Matrix<double, 3, 3>& R){
    R_matrix = R;
    R_matrix_set = true;
}
//-----------------------------------//
...
void motor::update_pos(Vector3d ref, const Matrix<double, 3, 3>& R){
    if(R_matrix_set){
        cerr << ORANGE << "Warning: " << RESET << "Rotation matrix was already set during this cycle. Ensure the values are not being set twice.\n \n";
    }
    R_matrix = R;
...
void motor::update_ori(const Matrix<double, 3, 3>& R){
    if(R_matrix_set){
        cerr << ORANGE << "Warning: " << RESET << "Rotation matrix was already set during this cycle. Ensure the values are not being set twice.\n \n";
    }
    R_matrix = R;
...
void motor::update_pos_ori(Vector3d ref, const Matrix<double, 3, 3>& R){
    if(R_matrix_set){
        cerr << ORANGE << "Warning: " << RESET << "Rotation matrix was already set during this cycle. Ensure the values are not being set twice.\n \n";
    }
    R_matrix = R;
