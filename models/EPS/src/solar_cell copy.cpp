...
{
    ref_pos << ref_point[0], ref_point[1], ref_point[2];
}
...
{
    if(lock_axis == 1)
        ref_normal_vec << 0, ref_o[0], ref_o[1];
    else if (lock_axis == 2)
        ref_normal_vec << ref_o[0], 0, ref_o[1];
    else
        ref_normal_vec << ref_o[0], ref_o[1], 0;
}
...
{
    R_matrix << R[0][0], R[0][1], R[0][2],
                R[1][0], R[1][1], R[1][2],
                R[2][0], R[2][1], R[2][2];
}

void solar_cell::update_R_matrix(const Matrix<double, 3, 3>& R)
...
void solar_cell::update_pos_ori(Vector3d ref, const Matrix<double, 3, 3>& R)
    //Description:    Updates the position and orientation of the solar cell based on reference position and rotation matrix.
    ...
    //Postconditions: ref_normal_vec is updated according to the clockwise rotation.
{
    ...
    R_Matrix2d << cos(rad), sin(rad),
                   -sin(rad), cos(rad);
    normal_vec_comp << ref_normal_vec[0], ref_normal_vec[2];
    normal_vec_comp = R_Matrix2d * normal_vec_comp;

    if(lock_axis == 1)
        ref_normal_vec << 0, normal_vec_comp[0], normal_vec_comp[1];
    else if (lock_axis == 2)
        ref_normal_vec << normal_vec_comp[0], 0, normal_vec_comp[1];
    else
        ref_normal_vec << normal_vec_comp[0], normal_vec_comp[1], 0;
}
    ...
    //Postconditions: Returns the calculated short-circuit current (I_sc).
{
    ...
    l_ray << light_ray[0], light_ray[1], light_ray[2];
    l_ray.normalize();
