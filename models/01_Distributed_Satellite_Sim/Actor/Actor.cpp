...
Actor::Actor(std::string name_) : name(std::move(name_)), running(false), sock(INVALID_SOCKET) {}
...
// Serialize Message to string: sender|id|replyTo|content\n
static std::string serializeMessage(const Message& msg) {
    return msg.sender + "|" + std::to_string(msg.id) + "|" + std::to_string(msg.replyTo) + "|" + msg.content + "\n";
}
...
bool Actor::sendNetworkMessage(const Message& msg) {
    ...
    while (sent < data.size()) {
        int ret = send(sock, data.c_str() + sent, static_cast<int>(data.size() - sent), 0);
